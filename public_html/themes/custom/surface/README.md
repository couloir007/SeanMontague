...
* `npm run build`: This is the command that runs `vite build` and `storybook build -o storybook` which compiles the theme and generates a static version of Storybook for prodcution.
...
* `ddev surface-build`: This command will run `npm run vite:build`` in the docker container.
* `ddev surface-watch`: This command will run `npm run watch`` in the docker container and will run the storybook server in the container.  It is slower than developing on your local machine.
